...
class ZerodhaKiteDataDisplay:
    ...
    def _flatten_tick_data(self, tick_data: Dict[str, Any]) -> Dict[str, Any]:
        """Flattens nested dictionary fields (like depth, ohlc) into single-level keys."""
        flat = {}
        for key, value in tick_data.items():
            if isinstance(value, dict):
                # Flatten nested dicts (e.g., 'ohlc_open', 'ohlc_high')
                for sub_key, sub_value in value.items():
                    flat[f"{key}_{sub_key}"] = sub_value
            elif key == 'depth' and isinstance(value, dict):
                # Specifically handle depth (top 5 bids/asks)
                for side in ['buy', 'sell']:
                    depth_list = value.get(side, [])
                    for i, level in enumerate(depth_list):
                        # e.g., 'depth_buy_0_price', 'depth_buy_0_quantity'
    ...
    def process_tick_data(self, tick_data):
        ...
        try:
            ...
            # print("-" * 30)
            # with open("tick_data_log_2.json", "a") as f:
            #     f.write(json.dumps(tick_data, indent=2, default=str) + "\n")
            # Store tick data as excel
            print("Storing tick data to Excel/CSV...")
            self.store_tick_data_excel(tick_data)
            # Store in buffer
            self.tick_buffer.append(tick_data)
            print("Tick data stored in buffer.")
            # Get symbol from token
            symbol = self.get_symbol_from_token(tick_data.get('instrument_token'))
            ...
            # #The bar aggregator updates the internal state for that token (High, Low, Current Close).
            #self.aggregate_to_bar(tick_data)

            # 2. Check for completed OHLCV bar
            closed_bar = self.aggregate_to_bar(tick_data)
            print("Closed Bar:", closed_bar)
            if closed_bar:
                logger.info(f"📊 {closed_bar['symbol']} 5m Bar Closed: {closed_bar['close']:.2f}, Volume: {closed_bar['volume']:,}")
                
            #     # 3. Trigger the OHLC callbacks with the completed bar data
                self._trigger_callbacks('ohlc', closed_bar)
            #print("$" * 30) 
            #print("End of Tick Processing") 
        except Exception as e:
...
def main():
    ...
    try:
        ...
        data_display = ZerodhaKiteDataDisplay(kite_ticker)
        ...
        data_display.register_depth_callback(demo_depth_callback)
        data_display.register_error_callback(demo_error_callback)
        data_display.register_connect_callback(demo_connect_callback)
        
        # Example tokens (you need to get actual tokens from Kite Connect API)
        # These are example tokens - replace with actual tokens
        #example_tokens = [912129,	3861249, 81153,	2426881, 3785729, 2674433]
