"""
Hourly Data Consolidator - Consolidates hourly metrics across dates and calculates technical indicators
Calculates: EMA, ATR, ATR_EMA, RSI
"""
...
class HourlyConsolidatorConfig:
    # Input/Output paths
    HOURLY_DATA_ROOT = r"F:\working\2024\Zerodha\breeze_data_service\metrics_rollup\hourly"
    OUTPUT_ROOT = r"F:\working\2024\Zerodha\breeze_data_service\metrics_rollup\consolidated_hourly"
    
    # Technical Indicator Parameters
    EMA_PERIODS = [5, 9, 14]           # Hourly EMA windows
    ATR_PERIOD = 14                     # ATR calculation period
    ATR_EMA_PERIOD = 20                 # EMA of ATR
    RSI_PERIOD = [6,9,14]                      # RSI calculation period
...
def load_instrument_data_across_dates(instrument_token, dates, root_path):
    """
    Load hourly data for a single instrument across all specified dates
    """
    dfs = []
    ...
    if 'trading_date' in combined_df.columns:
    ...
    combined_df = combined_df.drop_duplicates(subset=['exchange_timestamp'], keep='last')
...
def calculate_all_indicators(df, config):
    ...
    required_cols = ['ohlc_high', 'ohlc_low', 'ohlc_close']
    ...
    # 1. Calculate EMAs on close price
    for period in config.EMA_PERIODS:
        column_name = f'EMA_{period}'
        df[column_name] = calculate_ema(close, period)
    
    # 2. Calculate ATR
    df['ATR'] = calculate_atr(high, low, close, config.ATR_PERIOD)
    
    # 3. Calculate ATR EMA (EMA of ATR)
    df['ATR_EMA'] = calculate_ema(df['ATR'], config.ATR_EMA_PERIOD)
    
    # 4. Calculate RSI
    for period in config.RSI_PERIOD:
        column_name = f'RSI_{period}'    
        df[column_name] = calculate_rsi(close, period)
...
def consolidate_instrument(instrument_token, dates, config):
    """
    Consolidate hourly data for a single instrument and calculate indicators
    """
...
#         existing_df['exchange_timestamp'] = pd.to_datetime(existing_df['exchange_timestamp'])
        
#         # Get dates already in file (extract from timestamp)
#         existing_dates = set(
#             existing_df['exchange_timestamp'].dt.strftime('%Y-%m-%d').unique()
#         )
...
#         # Recalculate all indicators on combined data
#         # Drop old indicator columns
#         indicator_cols = [col for col in combined_df.columns if col in [
#             'ATR', 'ATR_EMA', 'RSI'
#         ] or col.startswith('EMA_')]
#         combined_df = combined_df.drop(columns=indicator_cols, errors='ignore')
        
#         # Recalculate
#         combined_df = calculate_all_indicators(combined_df, config)
    ...
    if os.path.exists(output_file):
        ...
        if config.ALWAYS_RECALC_TODAY and today_date in existing_dates:
            ...
            existing_df = existing_df[
                existing_df['exchange_timestamp'].dt.strftime('%Y-%m-%d') != today_date
            ]
        ...
        indicator_cols = [col for col in combined_df.columns if col in [
            'ATR', 'ATR_EMA'
        ] or col.startswith('EMA_') or col.startswith('RSI_')]
        combined_df = combined_df.drop(columns=indicator_cols, errors='ignore')
...
def consolidate_all_instruments(config):
    """
    Main function to consolidate hourly data for all instruments
    """
    print("="*80)
    print("🚀 HOURLY DATA CONSOLIDATION ENGINE")
    print("="*80)
    ...
    # Get all instruments
    all_instruments = get_all_instruments(config.HOURLY_DATA_ROOT, all_dates)
...
def verify_consolidated_data(config, sample_instruments=5):
    ...
    for instrument_token in sample:
        ...
        print(f"   Timestamp Range: {df['exchange_timestamp'].min()} to {df['exchange_timestamp'].max()}")
        print(f"   Columns: {list(df.columns)}")
        
        # Show indicator statistics
        indicator_cols = ['EMA_5', 'EMA_9', 'EMA_14', 'ATR', 'ATR_EMA', 'RSI']
        available_indicators = [col for col in indicator_cols if col in df.columns]
        
        if available_indicators:
            print(f"\n   Indicator Statistics (Latest 5 rows):")
            print(df[['exchange_timestamp'] + available_indicators].tail(5).to_string(index=False))
...
def get_consolidated_summary(config):
    ...
    for file in tqdm(consolidated_files[:100], desc="Sampling files"):  # Sample first 100
        ...
        try:
            ...
            summary_data.append({
                ...
                'total_hours': len(df),
                'start_timestamp': df['exchange_timestamp'].min(),
                'end_timestamp': df['exchange_timestamp'].max(),
    ...
    print(f"   Average Hours per Instrument: {summary_df['total_hours'].mean():.1f}")
    print(f"   Timestamp Range: {summary_df['start_timestamp'].min()} to {summary_df['end_timestamp'].max()}")
    print(f"   Total Storage: {summary_df['file_size_mb'].sum():.2f} MB")
    print(f"   EMA Columns Present: {summary_df['has_ema_5'].mean() * 100:.1f}%")
    print(f"   ATR Columns Present: {summary_df['has_atr'].mean() * 100:.1f}%")
    print(f"   RSI Columns Present: {summary_df['has_rsi'].mean() * 100:.1f}%")
...
if __name__ == "__main__":
    ...
    config = HourlyConsolidatorConfig()
    
    print("="*80)
    print("📋 CONFIGURATION")
    print("="*80)
    print(f"Input Path: {config.HOURLY_DATA_ROOT}")
    print(f"Output Path: {config.OUTPUT_ROOT}")
    print(f"\n📊 Technical Indicators:")
    print(f"   EMA Periods: {config.EMA_PERIODS}")
    print(f"   ATR Period: {config.ATR_PERIOD}")
    print(f"   ATR EMA Period: {config.ATR_EMA_PERIOD}")
